package com.logAnalyzer.pagination;
		...
		if (!isFilterByEmpty()) {
			...
			while (fbit.hasNext()) {
				...
				if(null == fbsb) {
					...
					fbsb.append(SPACE)
							.append(BRKT_OPN)
								.append(CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, pair.getKey()))
									.append(LIKE_PREFIX)
		...
		if(!isSortByEmpty()) {
			...
			while (sbit.hasNext()) {
				...
				if(null == sbsb) {
					...
					sbsb.append(ORDER_BY).append(CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, pair.getKey())).append(SPACE).append(pair.getValue());
				} else {
					sbsb.append(COMMA).append(CaseFormat.UPPER_CAMEL.to(CaseFormat.LOWER_UNDERSCORE, pair.getKey())).append(SPACE).append(pair.getValue());
				}
			}
		}
